...
figure(101);  % plot convergence history of parameters, reduced chi^2, lambda
 ...
 subplot(212)
  semilogy( cvg_hst(:,1) , [ cvg_hst(:,n+2) cvg_hst(:,n+3)], '-o','linewidth',4)
   text(0.8*cvg_hst(max_it,1),0.2*cvg_hst(max_it,n+2), '\chi^2_\nu','FontSize',16 );%,'color','blue'
   text(0.8*cvg_hst(max_it,1),2.5*cvg_hst(max_it,n+3), '\lambda','FontSize',16 );%,'color',dkgrn
